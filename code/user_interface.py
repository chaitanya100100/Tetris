...
class UI(object):
    ...
    def scorer(self):
        ...
        text = self.small_font.render("Next : ", True, (0,0,255))
        rect = text.get_rect()
        ...
        text = self.small_font.render("Score : " + str(self.score), True, (0,0,255))
        rect = text.get_rect()
        rect.center = [(cols+int(side_space/2))*blockSize, (rows/2 - 1)*blockSize]
        self.screen.blit(text, rect)

        text = self.small_font.render("Level : " + str(self.level), True, (0,0,255))
        rect = text.get_rect()
        rect.center = [(cols+int(side_space/2))*blockSize, (rows/2)*blockSize]
        self.screen.blit(text, rect)

        text = self.small_font.render("Session Best : " + str(self.session_best), True, (0,0,255))
        rect = text.get_rect()
        rect.center = [(cols+int(side_space/2))*blockSize, (rows/2+2)*blockSize]
        self.screen.blit(text, rect)

        text = self.small_font.render("High Score : " + str(self.high_score), True, (0,0,255))
        rect = text.get_rect()
    ...
    def gameOver_screen(self):
        ...
        self.big_message("Game Over :(", (255,0,0), -2*(big_font_size+small_font_size))
        self.small_message("Your Score : " + str(self.score), (255,0,0), -(small_font_size+big_font_size))
        self.small_message("Session Best Score : " + str(self.session_best), (255,0,0), -(big_font_size))
        self.small_message("High Score : " + str(self.high_score), (255,0,0), (small_font_size-big_font_size))
        self.small_message("Q for Quit and C for continue", (255, 0, 0), small_font_size)
        pygame.display.update()

    def start_screen(self):
        self.screen.fill(colors[0])
        self.big_message("Tetris :)", (255,0,0), -7*(small_font_size))
        part = int((rows*blockSize)/10)
        self.small_message("left : <- left arrow key", (255,0,0), -2*(part))
        self.small_message("right : -> right arrow key", (255,0,0), -1*(part))
        self.small_message("down : v down arrow key", (255,0,0), 0)
        self.small_message("rotate : ^ up arrow key", (255,0,0), 1*(part))
        self.small_message("drop : space bar", (255,0,0), 2*(part))
        self.small_message("push : p", (255,0,0), 3*(part))
        self.small_message("Q for Quit and C for continue", (255, 0, 0), 4*part)
        pygame.display.update()
